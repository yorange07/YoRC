...
public:
    ...
    std::vector<std::vector<QString>>& getBaseUserList() {return _baseUserList; };
    void setBaseUserList();
    void logoutClient(int clientSocket);
    void banClient (int id, bool flag);//Бан
    void modClient (int id, bool flag);//Возведение в модераторы
...
private:
    ...
    std::vector<std::vector<QString>> _baseUserList;
