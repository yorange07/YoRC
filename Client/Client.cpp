...
void Client::startClient() {
    connectToServer("127.0.0.1", 12345);//Ïîäêëþ÷åíèå ê ñåðâåðó ñ ÷àòîì
    std::cout << "!!!";
    std::cout << _chatActivation;
    if (_chatActivation == true)
        showLoginMenu();//×àò ïðèâåòñòâóåò è ïîêàçûâàåò ìåíþ âõîäà/ðåãèñòðàöèè
...
void Client::connectToServer(const std::string& ipAddress, int port) {
    ...
    // Çàïóñêàåì öèêë ïîëó÷åíèÿ ñîîáùåíèé â îòäåëüíîì ïîòîêå
...
void Client::showChat() {
    ...
    if (otherMessageSuccess == true) {
        // Çàïðîñ óäîâëåòâîðåí è ìû ñáðàñûâàåì ôëàã 
        otherMessageSuccess = false;
        otherAttemptCompleted = false;

    }
    else {
        // Çàïðîñ íå óäîâëåòâîðåí
...
void Client::receiveLoop() {
    while (true) {
        // Ðåàëèçóéòå öèêë ïîëó÷åíèÿ ñîîáùåíèé îò ñåðâåðà
        std::string receivedMessage = receiveMessage();
        if (receivedMessage.empty()) {
            break;  // Åñëè ñåðâåð îòêëþ÷åí, çàâåðøàåì öèêë
        }

        // Îáðàáîòêà ïîëó÷åííîãî ñîîáùåíèÿ
